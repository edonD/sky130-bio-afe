#!/usr/bin/env python3
"""Fine sweep Ra/Rptat to minimize TC while maintaining all other margins."""
...
def run_eval(netlist):
    ...
    sup_f = os.path.join(BLOCK_DIR, "stc_sup")
    temp_f = os.path.join(BLOCK_DIR, "stc_temp")
    ctrl = f""".control
op
let vref_val = v(vref)
let idd = -i(VDD)
...
"""
    n += "\n" + ctrl
    fpath = os.path.join(BLOCK_DIR, "stc_test.cir")
    with open(fpath, 'w') as f:
        f.write(n)
    result = subprocess.run(["ngspice", "-b", fpath],
        capture_output=True, text=True, timeout=180, cwd=SKY130_DIR)
    out = result.stdout + "\n" + result.stderr
    vref = power = None
...
print("Fine sweep Ra/Rptat (N=8, fine-step temp evaluation)")
print(f"{'Ra':>5} {'Rp':>6} | {'vref':>7} {'tc':>6} {'psrr':>6} | {'vm':>5} {'tm':>5} {'pm':>5} {'min':>5}")
best = -999

for ra in [120, 121, 122, 123, 124]:
    for rp_10 in [120, 123, 125, 127, 130]:
        rp = rp_10 / 10.0
        n = set_params(base, {'p_ra_l': f'{ra}e-6', 'p_rptat_l': f'{rp}e-6'})
        r = run_eval(n)
        ...
        if mn > best:
            ...
            marker = ""
        if mn > 25:
            print(f"{ra:>5} {rp:>6} | {r['vref']:7.4f} {r['tc']:6.1f} {r['psrr']:6.1f} | {vm:5.1f} {tm:5.1f} {pm:5.1f} {mn:5.1f}{marker}")
